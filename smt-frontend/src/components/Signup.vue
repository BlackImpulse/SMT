<template>
  <div class="sign-up">
    <ul>
      <li class="sign-up-tab"><p>SIGN UP</p></li>
      <router-link to="/login"><li class="login-tab"><p>LOG IN</p></li></router-link>
    </ul>
    <form id="sign-up" @submit="checkForm">
      <div class="user-box">
        ...
        <p v-if="usernameError">The username is already used.</p>
      </div>
      <div class="user-box">
        <input @focusin="inputFocusIn"
               v-model="password"
...
export default {
  ...
  methods: {
    ...
    checkForm(e) {
      ...
      this.$store.dispatch('auth/register', new User(this.username, this.password)).then(
          () => {
...
.sign-up ul li.login-tab:hover {
  text-decoration-line: underline;
}

.sign-up ul li.sign-up-tab {
  width: 240px;
  height: 80px;
...
.sign-up form .user-box {
  position: relative;
  margin-bottom: 30px;
}

.sign-up form .user-box input{
  width: 390px;
  height: 64px;
